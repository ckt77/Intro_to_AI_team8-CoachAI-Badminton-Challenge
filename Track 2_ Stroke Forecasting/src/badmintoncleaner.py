...
class BadmintonDataset(Dataset):
    def __init__(self, matches, config):
        ...
        group = matches[['rally_id', 'ball_round', 'type', 'landing_x', 'landing_y', 'player_location_area', 'set', 'opponent_location_x', 'opponent_location_y']].groupby('rally_id').apply(lambda r: (r['ball_round'].values, r['type'].values, r['landing_x'].values, r['landing_y'].values, r['player_location_area'].values, r['set'].values, r['opponent_location_x'].values, r['opponent_location_y'].values))

        self.sequences, self.rally_ids = {}, []
        for i, rally_id in enumerate(group.index):
            ball_round, shot_type, landing_x, landing_y, player, sets, opponent_location_x, opponent_location_y = group[rally_id]
            self.sequences[rally_id] = (ball_round, shot_type, landing_x, landing_y, player, sets, opponent_location_x, opponent_location_y)
    ...
    def __getitem__(self, index):
        ...
        ball_round, shot_type, landing_x, landing_y, player, sets, opponent_location_x, opponent_location_y = self.sequences[rally_id]
        # landing_x = landing_x + opponent_location_x
        # landing_y = landing_y + opponent_location_y
        ...
        pad_input_opponent_location_x = np.full(self.max_ball_round, fill_value=PAD, dtype=float)
        pad_input_opponent_location_y = np.full(self.max_ball_round, fill_value=PAD, dtype=float)
        pad_output_opponent_location_x = np.full(self.max_ball_round, fill_value=PAD, dtype=float)
        pad_output_opponent_location_y = np.full(self.max_ball_round, fill_value=PAD, dtype=float)
        ...
        if len(ball_round) > self.max_ball_round:
            ...
            pad_input_opponent_location_x[:] = opponent_location_x[0:-1:1][:rally_len] 
            pad_input_opponent_location_y[:] = opponent_location_y[0:-1:1][:rally_len] 
            pad_output_opponent_location_x[:] = opponent_location_x[1::1][:rally_len] 
            pad_output_opponent_location_y[:] = opponent_location_y[1::1][:rally_len] 
        else:
            ...
            pad_input_opponent_location_x[:rally_len] = opponent_location_x[0:-1:1]
            pad_input_opponent_location_y[:rally_len] = opponent_location_y[0:-1:1]
        ...
        return (pad_input_shot, pad_input_x, pad_input_y, pad_input_player,
                ...
                rally_len, sets[0], pad_input_opponent_location_x, pad_input_opponent_location_y)
